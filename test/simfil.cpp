...
TEST_CASE("Model Functions", "[yaml.mode-functions]") {
    ...
    SECTION("Test split(... )") {
        ...
        REQUIRE_ERROR("split(panic(), '.')");
        REQUIRE_ERROR("split('a.b.c', panic())");
    }
    SECTION("Test select(... )") {
        ...
        REQUIRE_ERROR("select(panic(), 0)");
        REQUIRE_ERROR("select(0, panic())");
    }
    SECTION("Test sum(... )") {
        ...
        REQUIRE_ERROR("sum(panic(), 0)");
        REQUIRE_ERROR("sum(range(1, 10)..., panic())");
        REQUIRE_ERROR("sum(range(1, 10)..., 0, panic())");
    }
