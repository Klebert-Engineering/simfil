...
using simfil::FixedArrayArena;

TEST_CASE("FixedArrayArena basic functionality", "[FixedArrayArena]") {
    FixedArrayArena<int32_t, 24, 8> arena;
...
TEST_CASE("FixedArrayArena many arrays", "[FixedArrayArena]") {
    ...
    for (auto i = 1; i <= count + 1; ++i) {
        (void)handles.emplace_back(arena.newArray(size, i));
    }
...
TEST_CASE("FixedArrayArena index OOR", "[FixedArrayArena]") {
    FixedArrayArena<int32_t, 1, 31> arena;
...
TEST_CASE("FixedArrayArena size OOR", "[FixedArrayArena]") {
    FixedArrayArena<int32_t, 31, 1> arena;
...
TEST_CASE("FixedArrayArena handle size", "[FixedArrayArena]") {
    REQUIRE(sizeof(FixedArrayArena<int32_t,  4,  4>::Handle) == 1);
    REQUIRE(sizeof(FixedArrayArena<int32_t,  8,  8>::Handle) == 2);
    REQUIRE(sizeof(FixedArrayArena<int32_t, 31,  1>::Handle) == 4);
    REQUIRE(sizeof(FixedArrayArena<int32_t, 31, 33>::Handle) == 8);
}

TEST_CASE("FixedArrayArena bitsery", "[FixedArrayArena]") {
    FixedArrayArena<int32_t, 16, 16> arena;
