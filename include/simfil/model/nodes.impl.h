...
template <class ModelType, class ModelNodeType>
BaseObject<ModelType, ModelNodeType>::BaseObject(ArrayIndex members, ModelConstPtr pool_, ModelNodeAddress a)
    : MandatoryDerivedModelNodeBase<ModelType>(std::move(pool_), a), storage_(nullptr), members_(members)
{
    storage_ = &model().objectMemberStorage();
}
...
template <class ModelType, class ModelNodeType>
BaseObject<ModelType, ModelNodeType>& BaseObject<ModelType, ModelNodeType>::addField(std::string_view const& name, ModelNode::Ptr const& value) {
